  ...
  return (
    <Card className="border-border/60 shadow-sm">
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-semibold text-foreground">Flujo del proceso</CardTitle>
      </CardHeader>
      <CardContent>
          ...
          {STEPS.map((step, i) => (
            <div key={i} className="flex flex-1 flex-col items-center">
              ...
              <div className="flex w-full items-center">
                ...
                <div
                  className={cn(
                    'relative flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-white ring-4',
                    step.color,
                    step.ring
                  )}
                >
                  <step.icon className="h-5 w-5" />
                  <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-background border border-border text-[9px] font-bold text-foreground">
                    {step.number}
                  </span>
                </div>
